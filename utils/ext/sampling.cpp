...
py::array_t<int> sample_negative(std::vector<int> users, int item_num, py::dict allPos, int neg_num)
{
    int row = neg_num;
    int col = users.size();
    ...
    for (int user_i = 0; user_i < users.size(); user_i++)
    {
        ...
        // py::list pos_item_pylist = ;
        std::vector<int> pos_item = allPos[py::int_(user)].cast<std::vector<int>>();
        int negitem = 0;

        for (int neg_i = 0; neg_i < row; neg_i++)
        {
            do
            {
